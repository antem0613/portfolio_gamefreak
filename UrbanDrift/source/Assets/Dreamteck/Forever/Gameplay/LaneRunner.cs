namespace Dreamteck.Forever
{
    ...
    public class LaneRunner : Runner
    {
        ...
        public int lane
        {
            get { return _lane; }
            set {
                if (value > laneCount) value = laneCount - 1;
        ...
        protected override void OnFollow(SplineSample followResult)
        {
            ...
            if (useCustomPaths) //Custom lane following
            {
                ...
                {
                    if (lane > _segment.customPaths.Length - 1) lane = _segment.customPaths.Length - 1;
                    if (laneValue > _segment.customPaths.Length - 1) laneValue = _segment.customPaths.Length - 1;
                    if (lastLane > _segment.customPaths.Length - 1) lastLane = _segment.customPaths.Length - 1;
                    _result = customPathResults[lastLane];
                    _result.Lerp(ref customPathResults[lane], Mathf.Abs(laneValue - lastLane));
