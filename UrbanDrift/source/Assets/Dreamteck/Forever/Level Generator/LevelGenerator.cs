...
namespace Dreamteck.Forever
{
    ...
    public class LevelGenerator : MonoBehaviour
    {
        ...
        private List<GameObject> _generatedCity = new List<GameObject>();
        private Direction correctWay = Direction.None;
        ...
        private List<LevelSegment> _branchSegments = new List<LevelSegment>();
        ...
        IEnumerator StartRoutine()
        {
            ...
            while (isLoadingLevel || !GameManager.instance.IsInitialized)
            {
                yield return null;
            }

            GameManager.instance.trafficArgs.Clear();
            ...
            while (segments.Count <= 0 || !segments[0].isReady)
            {
                yield return null;
            }
        ...
        {
            Debug.Log($"Level {index} is removed");
        ...
        private IEnumerator ClearRoutine()
        {
            ...
            for (int i = 0; i < _branchSegments.Count; i++)
            {
                _branchSegments[i].Destroy();
            }
        ...
        {
            foreach(var b in _generatedCity)
            {
                DestroyImmediate(b);
            }
        ...
        public double Travel(double start, float distance, Spline.Direction direction)
        {
            ...
            for (int i = 0; i < segments.Count; i++)
            {
                iterations += segments[i].mainPath.spline.iterations;
            }
        ...
        private IEnumerator CreateSegment(System.Action completeHandler)
        {
            ...
            LevelSegment[] branchSegments = null;
            ...
            {
                if (_segments.Count > 0 && _segments[_segments.Count - 1].paths.Count > 0 && !_segments[_segments.Count - 1].isBranchSetUp)
                {
                    branchSegments = new LevelSegment[_segments[_segments.Count - 1].paths.Count];
                    branchSegments = currentLevel.InstantiateSegmentOnJunction(_segments[_segments.Count - 1].paths.Count);
                    _segments[_segments.Count - 1].isBranchSetUp = true;
                }
                else if(_branchSegments.Count<=0)
                {
                    segment = currentLevel.InstantiateSegment();
                }
            }

            if (branchSegments != null)
            {
                for(int i = 0; i < branchSegments.Length; i++)
                {
                    yield return InitializeSegmentRoutine(branchSegments[i] , i);
                }
            }
            else if (segment != null)
            {
                ...
                if(segment.tileObjectManager.trafficLightSystem != null)
                {
                    GameManager.instance.trafficLightSystems.Add(segment.tileObjectManager.trafficLightSystem);
                }
            }
        ...
        private IEnumerator CreateSegment(LevelSegment segmentPrefab, System.Action completeHandler)
        {
            ...
            if (_segments[_segments.Count - 1].paths.Count > 0)
            {
                for (int i = 0; i< _segments[_segments.Count - 1].paths.Count; i++)
                {
                    LevelSegment segment = Instantiate(segmentPrefab);
                    yield return InitializeSegmentRoutine(segment , i);
                }
            }
            else if(_branchSegments.Count <= 0)
            {
        ...
        private IEnumerator InitializeSegmentRoutine(LevelSegment segment)
        {
            ...
            if(IsActiveCity && currentLevelIndex == 1 && segment.segmentKind == 1)
            {
                    Vector3 offset = new Vector3(55f, 0, 0);
                    _generatedCity.Add(Instantiate(cityBlocks[Random(0, cityBlocks.Count - 1)], segmentTrs.position + (segmentTrs.rotation * offset), Quaternion.AngleAxis(90f * Random(0, 3), Vector3.up)));
                    _generatedCity.Add(Instantiate(cityBlocks[Random(0, cityBlocks.Count - 1)], segmentTrs.position - (segmentTrs.rotation * offset), Quaternion.AngleAxis(90f * Random(0, 3), Vector3.up)));

                    if (_generatedCity.Count > 4)
                    {
                        Destroy(_generatedCity[0]);
                        Destroy(_generatedCity[1]);
                        _generatedCity.RemoveRange(0, 2);
                    }
            }
            ...
            if (_segments.Count > 0 && segment.segmentKind!=2)
            {
                segment.tileObjectManager.UpdateJoints();
                segment.tileObjectManager.SetUp(GameManager.instance.GetNextArg());
        ...
        private IEnumerator InitializeSegmentRoutine(LevelSegment segment , int index)
        {
            segment.gameObject.SetActive(false);
            Transform segmentTrs = segment.transform;
            Vector3 spawnPos = segmentTrs.position;
            ...
            if (segments.Count > 0)
            {
                ...
                if (_segments[_segments.Count - 1].paths.Count > 0)
                {
                    _segments[_segments.Count - 1].Evaluate(index, 1.0, ref lastSegmentEndResult);
                }
                else
                {
                    _segments[_segments.Count - 1].Evaluate(1.0,ref lastSegmentEndResult);
            ...
            segment.isBranchSetUp = true;

            _branchSegments.Add(segment);
            if (onSegmentCreated != null)
            {
                onSegmentCreated(segment);
            }
        ...
        private IEnumerator SwitchLane(int lane)
        {
            _segments[_segments.Count - 1].customExit = _segments[_segments.Count - 1].exits[lane];

            if (lane > 0)
            {
                _segments[_segments.Count - 1].customMainPath = lane;

                if (IsActiveCity && _generatedCity.Count >= 4)
                {
                    if (lane == 1)
                    {
                        _generatedCity.Swap(0, 3);
                        _generatedCity.Swap(2, 3);
                    }
                    else
                    {
                        _generatedCity.Swap(1, 2);
                        _generatedCity.Swap(2, 3);
                    }
                }
            }

            _branchSegments.Swap(_branchSegments.Count - 1, _branchSegments.Count - 3 + lane);

            for(int i = 0; i < _branchSegments.Count; i++)
            {
                _branchSegments[i].Setup(this, segmentIndex++);
            }

            LevelSegment segment = _branchSegments[_branchSegments.Count - 1];

            currentPathGenerator.GeneratePath(segment);
            _segments.AddRange(_branchSegments);
            _branchSegments.Clear();
            _segments.Swap(_segments.Count - 2, _segments.Count - 4);
            ...
            {
                case 0:
                    mainLane = (int)Direction.Forward; break;
                case 1:
                    mainLane = (int)Direction.Right; break;
                case 2:
                    mainLane = (int)Direction.Left; break;
            }
        }
        ...
        public void ChangeLane(int lane)
        {
            if (_segments[_segments.Count - 1].paths.Count > 0)
            {
                StartCoroutine(SwitchLane(lane));
            }
        }

        IEnumerator WaitForTutorial()
        {
            while (!GameManager.instance.endTutorial)
            {
                yield return null;
            }
        ...
        private void OnSegmentEntered(LevelSegment entered)
        {
            ...
            if(entered.segmentKind != 0)
            {
                GameManager.instance.HasInput = false;
                ...
                {
                    if (entered.segmentKind == 1)
                    {
                        GameManager.instance.IsSliderActivated = true;
                    }
                }
            }
            else
            {
                if (!GameManager.instance.DontFail)
                {
                    GameManager.instance.IsGameOver = ((correctWay & (Direction)mainLane) == 0);
            ...
            for (int i = 0; i < _segments.Count; i++)
            {
                if (_segments[i].index == _enteredSegmentIndex)
                {
                    ...
                    if (segmentsAhead < _generateSegmentsAhead && _branchSegments.Count <= 0)
                    {
