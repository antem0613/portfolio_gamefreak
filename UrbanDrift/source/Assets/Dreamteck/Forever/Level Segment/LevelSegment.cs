...
namespace Dreamteck.Forever
{
    ...
    public partial class LevelSegment : MonoBehaviour
    {
        ...
        public int segmentKind;
        public bool isBranchSetUp = false;
        ...
        [HideInInspector]
        public UrbanDrift.TileObjectManager tileObjectManager;
        ...
        public SplinePath mainPath = new SplinePath();

        [System.NonSerialized]
        public List<SplinePath> paths = new List<SplinePath>();
        ...
        void OnOriginOffset(Vector3 dir)
        {
            ...
            for (int i = 0; i < mainPath.samples.Length; i++)
            {
                mainPath.samples[i].position -= dir;
            }
            if (mainPath != null && mainPath.spline != null)
            {
                for (int i = 0; i < mainPath.spline.points.Length; i++)
                {
                    mainPath.spline.points[i].position -= dir;
                    mainPath.spline.points[i].tangent -= dir;
                    mainPath.spline.points[i].tangent2 -= dir;
                }
            }
        ...
        public void Setup(LevelGenerator generator, int index)
        {
            ...
            if (type == Type.Custom)
            {
                ...
                {
                    paths.Add(new SplinePath());
                }
                    
                for (int i = 0;i<paths.Count;i++)
                {
                    paths[i].samples = new SplineSample[2];
                    paths[i].samples[0] = new SplineSample(customEntrance.position, customEntrance.up, customEntrance.forward, Color.white, 1f, 0.0);
                    paths[i].samples[1] = new SplineSample(exits[i].position, exits[i].up, exits[i].forward, Color.white, 1f, 1.0);
                }
                ...
                {
                    mainPath.samples = new SplineSample[2];
                    mainPath.samples[0] = new SplineSample(customEntrance.position, customEntrance.up, customEntrance.forward, Color.white, 1f, 0.0);
                    mainPath.samples[1] = new SplineSample(customExit.position, customExit.up, customExit.forward, Color.white, 1f, 0.0);
                }
        ...
        public void Evaluate(double percent, ref SplineSample result, SplinePath.EvaluateMode mode = SplinePath.EvaluateMode.Cached)
        {
            if (UseCustomPath())
            {
                Debug.Log("CustomPathToJunc " + _customMainPath);
                customPaths[_customMainPath].Evaluate(percent, ref result, mode);
                return;
            }
            mainPath.Evaluate(percent, ref result, mode);
        }

        public void Evaluate(int index ,double percent, ref SplineSample result, SplinePath.EvaluateMode mode = SplinePath.EvaluateMode.Cached)
        {
            if (UseCustomPath())
            {
                Debug.Log("CustomPath " + index);
                customPaths[index].Evaluate(percent, ref result, mode);
                return;
            }
            paths[index].Evaluate(percent, ref result, mode);
        }
        ...
        public Vector3 EvaluatePosition(double percent, SplinePath.EvaluateMode mode = SplinePath.EvaluateMode.Cached)
        {
            ...
            return mainPath.EvaluatePosition(percent, mode);
        }
        ...
        public virtual float CalculateLength(double from = 0.0, double to = 1.0, SplinePath.EvaluateMode mode = SplinePath.EvaluateMode.Cached)
        {
            ...
            return mainPath.CalculateLength(from, to, mode);
        }
        ...
        public double Travel(double start, float distance, Spline.Direction direction, out float traveled, SplinePath.EvaluateMode mode = SplinePath.EvaluateMode.Cached)
        {
            ...
            return mainPath.Travel(start, distance, direction, out traveled, mode);
        }
        ...
        public virtual void Project(Vector3 point, ref SplineSample result, double from = 0.0, double to = 1.0, SplinePath.EvaluateMode mode = SplinePath.EvaluateMode.Cached)
        {
            ...
            mainPath.Project(point, ref result, from, to, mode);
        }
