    ...
    public partial class LevelSegment : MonoBehaviour
    {
        ...
        public virtual IEnumerator OnBeforeExtrude()
        {
            ...
            if(type == Type.Custom)
            {
                ...
                mainPath.samples[1] = new SplineSample(customExit.position, customExit.up, customExit.forward, Color.white, 1f, 1.0);
            }
        }
        ...
        public void Extrude()
        {
            if (type == Type.Custom)
            {
                ...
                return;
            } else mainPath.CalculateSamples();
            
            //Make the first result the same as the previous one to prevent seams (stitch)
            if (stitch && _previous != null)
            {
                mainPath.samples[0] = _previous.mainPath.samples[_previous.mainPath.samples.Length - 1];
                mainPath.samples[0].percent = 0.0;
            }
