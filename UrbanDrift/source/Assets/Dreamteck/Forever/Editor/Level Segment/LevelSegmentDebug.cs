namespace Dreamteck.Forever.Editor
{
    ...
    public static class LevelSegmentDebug
    {
        ...
        public static void DrawGeneratedSpline(LevelSegment segment)
        {
            ...
            //Debug spline points
            if (segment.mainPath != null)
            {
                for (int i = 0; i < segment.mainPath.spline.points.Length; i++)
                {
                    Vector3 pos = segment.mainPath.spline.points[i].position;
                    float handleSize = HandleUtility.GetHandleSize(segment.mainPath.spline.points[i].position);
                    Handles.DrawSolidDisc(pos, cameraPos - pos, handleSize * 0.1f);
                    Handles.DrawLine(pos, pos + segment.mainPath.spline.points[i].normal * segment.drawSampleScale);
                    if (segment.mainPath.spline.type == Spline.Type.Bezier)
                    {
                        Handles.DrawDottedLine(segment.mainPath.spline.points[i].position, segment.mainPath.spline.points[i].tangent, 10f);
                        Handles.DrawDottedLine(segment.mainPath.spline.points[i].position, segment.mainPath.spline.points[i].tangent2, 10f);
                        Handles.DrawWireDisc(segment.mainPath.spline.points[i].tangent, cameraPos - segment.mainPath.spline.points[i].tangent, handleSize * 0.075f);
                        Handles.DrawWireDisc(segment.mainPath.spline.points[i].tangent2, cameraPos - segment.mainPath.spline.points[i].tangent2, handleSize * 0.075f);
                    }
                }
            }
        }
        ...
        public static void DrawGeneratedSamples(LevelSegment segment)
        {
            ...
            for (int i = 0; i < segment.mainPath.samples.Length; i++)
            {
                Vector3 pos = segment.mainPath.samples[i].position;
                Vector3 right = segment.mainPath.samples[i].right;
                Vector3 normal = segment.mainPath.samples[i].up;
                float size = segment.mainPath.samples[i].size;
